  ...
  useEffect(()=>{
    onAuthStateChanged(auth,async (user)=>{
      if (user) {
        ...
        if (docSnap.data().avtara) {
          setPrevImage(docSnap.data().avtara)
        }
      }
      else{
        navigate('/')
     ...
     <div className='profile'>
      <div className="profile-container">
        <form onSubmit={profileUpdate} >
          ...
          <label htmlFor="avtara">
            <input onChange={(e)=>{setImage(e.target.files[0])}} type="file" id='avtara' accept='.png, .jpg , .jpeg'hidden />
